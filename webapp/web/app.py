...
def get_logged_in_user():
    if "user" not in session.keys():
        ...
        user = session['user']
    return jsonify({"user" : session["email"], "reviewer_id" : user[2][0]})
...
# get random product by category
@app.route('/get_bpr_product_by_category', methods=['POST'])
def get_bpr_product_by_category():
    ...
    db = DBUtil()
    r = db.get_bpr_recommended_item(asin_list[u'Recommendation list'], category, limit)
    ...
    # return jsonify([reviewer_id, category, limit])
...
@app.route('/get_reviewers2/<int:top>/')
def get_reviewers2(top):
    retval = {}
    for x in bprmodel.keys()[0:top]:
        asin_list = bprmodel[x]
        l = asin_list["Recommendation list"]
        retval[x] = {
                    "count" : len(l),
                    "asin" : l
                    }
...
@app.route('/get_asin_count_by_reviewer_id/<string:reviewer_id>/')
def get_asin_count_by_reviewer_id(reviewer_id):
    if reviewer_id in bprmodel.keys():
      asin_list = bprmodel[reviewer_id]
    l = asin_list["Recommendation list"]
